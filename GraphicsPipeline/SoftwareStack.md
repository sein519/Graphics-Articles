...
- The compiler usually exists as a module that is separate from the API runtime
    ...
    - An app can also perform online compilation through a compiler supplied as a library (e.g. glslang)
...
- Note: Some APIs, such as OpenGL, compile shader code at the user-mode driver (UMD)
...
- Back-end implementation of the API runtime
...
- Features that are rarely used may not be HW-accelerated by the GPU and thus can be emulated by the UMD
    - It might emulate features like texture border by injecting additional instructions into the user's shader code
...
- Manages memory allocation
    ...
    - To minimize allocation overhead, requests are handled via suballocation from this preallocated memory
        - Explicit APIs like Vulkan and D3D12 may delegate memory management responsibilities to the app
        - However, opaque interfaces such as descriptor pools remain managed by the UMD
- Creates and submits indirect buffers(IB)
    ...
    - IB memory blocks allocated by the KMD are suballocated by the UMD
    - IBs typically reside in mapped memory and, once prepared, are submitted to the KMD
        - The KMD passes only the pointer of IB to the CP
        - Before execution, the CP resolves IB contents through DMA reads
...
- Submits command packet
    - Compose command packets that the CP can execute immediately
    - Submit system commands required for GPU operation
    - IBs supplied by the UMD are converted into jump packets by the KMD, then forwarded to the CP